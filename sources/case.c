...
int		search_valid_cases(t_game **game, int map_i, int map_j)
{
	...
	assign_null(&i, &j, &opp_imposition, &my_imposition);
	while (i < (*game)->piece->size_y)
	{
		...
		{
			if (check_sign(*game, (*game)->map->arr[map_i][map_j],
			(*game)->piece->arr[i][j]) == 1)
				my_imposition++;
			else if (check_sign(*game, (*game)->map->arr[map_i][map_j],
			(*game)->piece->arr[i][j]) == -1)
				opp_imposition++;
			j++;
			map_j++;
	...
	return (my_imposition == 1 && opp_imposition == 0);
}
...
int		min_distance_in_piece(int j_map, int i_map, t_game *game)
{
	...
	while (i < piece->size_y)
	{
		...
		{
			if (piece->arr[i][j] == '*' &&
			(game->distance_map[i_map + i][j_map + j] < min_dist || !min_dist))
				min_dist = game->distance_map[i_map + i][j_map + j];
			j++;
		}
		i++;
	}
